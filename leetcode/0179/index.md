...
首先想到按字典序降序拼接，但是某些情况并不适用。例如 [3,30,34,5,9] 的字典序降序为 [9,5,34,30,3]，但正确的顺序应该是 [9,5,34,3,30]。
因此需要重新定义优先顺序。

这里有个巧妙的想法，只要 $\overline{AB}>\overline{BA}$，就认为 A 的优先级更高，应该排在前面。
直觉告诉我们这样很可能是正确的，不过需要证明一下。

可以采用反证法。假设最大结果 res 中存在 $\overline{AB}>\overline{BA}$ 但 B 排在 A 的前面。
...
设 A、B、C 对应的字符串长度分别为 x、y、z，那么由 $\overline{BC} \geq \overline{CB}，\overline{CA} \geq \overline{AC}$ 可以推得：
...
def largestNumber(self, nums):
	compare = lambda x,y: 0 if x+y==y+x else -1 if x+y>y+x else 1
