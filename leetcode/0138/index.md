...
最直接的就是用哈希表记录原节点和新节点的映射，遍历时已存在节点就指向它，不存在就新建节点并添加到哈希表中。
...
先将新节点 new_node 依次插入到原节点 old_node 的后面。那么 old_node.random.next 就应该是 new_node 的 random 指针指向的节点。
所以可以遍历设置新节点的 random 指针。
