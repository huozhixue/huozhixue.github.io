...
给定一个表达式 expression 如 expression = "e + 8 - a + 5" 和一个求值映射，如 {"e": 1}（给定的形式为 evalvars = ["e"] 和 evalints = [1]），
...
计算表达式的通用解法可以参考 {{< lc "0224" >}}。区别在于本题存在变量。

考虑用字典来表示带变量的表达式，对于每一项，用变量的有序元组作为 key，系数作为 value。
比如 5 + a * b + bb * cc 表示为 {(): 5, (a, b): 1, (bb, cc): 1}。

这样就可以计算两个带变量的表达式的加减乘。如果用 python 的 Counter()，可以进一步简化加减法。

计算完毕后，将最终的字典按 (元组长度, 元组本身) 排序，依次转为要求的字符串格式即可。
...
def basicCalculatorIV(self, expression: str, evalvars: List[str], evalints: List[int]) -> List[str]:
	def mul(A, B):
		tmp = A.copy()
		A.clear()
		for a, b in product(tmp, B):
			A[tuple(sorted(a+b))] += tmp[a] * B[b]

	func = {'+': Counter.update, '-': Counter.subtract, '*': mul}
	eval_d, pro = dict(zip(evalvars, evalints)), dict(zip('(+-*', [0, 1, 1, 2]))
	stack, ops = [], []
	for token in re.findall(r'[a-z0-9]+|[-+*()]', expression + '+'):
		if token in '+-*':
			...
			token = str(eval_d.get(token, token))
			stack.append(Counter({(token,): 1} if token.isalpha() else {(): int(token)}))
	return ['*'.join((str(v),)+k) for k, v in sorted(stack[0].items(), key=lambda x: (-len(x[0]), x[0])) if v != 0]
```
