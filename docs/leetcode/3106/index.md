...
> <u>**[力扣第 392 场周赛第 2 题](https://leetcode.cn/problems/lexicographically-smallest-string-after-operations-with-constraint/)**</u>
...
**相似问题：**
- [2734：执行子串操作后的字典序最小字符串（1405 分）](/leetcode/2734)


## 分析

典型的贪心，尽可能的将前面的改为'a'，不够用了就改为尽可能小的并退出。
...
class Solution:
    def getSmallestString(self, s: str, k: int) -> str:
        ...
        for i,c in enumerate(s):
            w = min(ord(c)-ord('a'),26-ord(c)+ord('a'))
            if k<w:
                res[i]= chr(ord(c)-k)
                break
            res[i] = 'a'
