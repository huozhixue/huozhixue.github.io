...
> <u>**[力扣第 375 场周赛第 2 题](https://leetcode.cn/problems/double-modular-exponentiation/)**</u>
...
class Solution:
    def getGoodIndices(self, variables: List[List[int]], target: int) -> List[int]:
        def check(p):
            return pow(pow(p[0],p[1],10),p[2],p[3])==target
        return [i for i,p in enumerate(variables) if check(p)]
```
36 ms
