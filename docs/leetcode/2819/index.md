...
> <u>**[力扣第 2819 题](https://leetcode.cn/problems/minimum-relative-loss-after-buying-chocolates/)**</u>
...
- 针对询问 k,m，应该选离 k 最远的 m 个数
- 因此将价格排序，每次必然选一段前缀加一段后缀
- 枚举前缀长度，所求损失必然先降再升
- 因此二分查找到谷底，计算损失即可
...
class Solution:
    def minimumRelativeLosses(self, prices: List[int], queries: List[List[int]]) -> List[int]:
        def cal(k,m):
            def f(i):
                return p[i]+2*k*(m-i)-p[-1]+p[-1-m+i]
            i = bisect_left(range(m),True,key=lambda i:f(i)<f(i+1))
            return f(i)

        n = len(prices)
        prices.sort()
        p = list(accumulate([0]+prices))
        return [cal(k,m) for k,m in queries]
```
2023 ms
