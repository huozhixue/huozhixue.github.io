...
> <u>**[力扣第 271 场周赛第 3 题](https://leetcode.cn/problems/watering-plants-ii/)**</u>
...
**相似问题：**
- [2079：给植物浇水（1320 分）](/leetcode/2079)


## 分析

模拟，数组长度奇数时特殊判断中间元素即可。
...
class Solution:
    def minimumRefill(self, plants: List[int], capacityA: int, capacityB: int) -> int:
        def cal(A,ma):
            ...
            return res,s
        n = len(plants)
        a,s1 = cal(plants[:n//2],capacityA)
        b,s2 = cal(plants[(n+1)//2:][::-1],capacityB)
        return a+b+(n%2 and max(s1,s2)<plants[n//2])
```
83 ms
