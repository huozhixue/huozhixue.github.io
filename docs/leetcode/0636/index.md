...
> <u>**[力扣第 636 题](https://leetcode.cn/problems/exclusive-time-of-functions/)**</u>
...
用栈模拟过程。函数开始时入栈，结束时出栈。

为了方便，将 end 对应的时间加 1，相邻两条记录之间的时间差便是栈顶函数运行的一段单位时间。
...
def exclusiveTime(self, n: int, logs: List[str]) -> List[int]:
    ...
    for log in logs:
        i, op, ti = log.split(':')
        ti = int(ti) if op=='start' else int(ti)+1
        if stack:
            res[stack[-1]] += ti - prev
        if op == 'start':
            stack.append(int(i))
