...
class Theme {
    ...
    initSearch() {
        ...
        const initAutosearch = () => {
            ...
            autosearch.on('autocomplete:selected', (_event, suggestion, _dataset, _context) => {
                ...
                document.body.classList.remove('blur');
				$.pjax({url: suggestion.uri, container: '.container', fragment: '.container', timeout: 8000});
				window.config.data = {}; window.config.media={}; window.config.typeit.data = {};
				this.init2();
            });
    ...
    initHeaderLink() {
        for (let num = 1; num <= 6; num++) {
            this.util.forEach(document.querySelectorAll('.single .content > h' + num), $header => {
				...
				$header.innerHTML = `<a href="#${$header.id}" class="header-mark"></a>${$header.innerText}`;
            });
        }
    }
    ...
    initComment() {
        if (this.config.data.valine){
            this.config.data.valine['path'] = window.location.pathname;
            new Valine(this.config.data.valine);
        }
    ...
    switchMedia($media, $link, curData, type){
        $link.innerText = curData[1];
		$link.href = curData[2];
        $media.removeChild($media.lastElementChild);
        let player = ``;
        if (type=='music') {
            player = `<meting-js auto="${curData[0]}" list-max-height="263px" order="random"></meting-js>`;
        }
		else {
			player = `<div class="bilibili"><iframe src="${curData[0]}" scrolling="no" border="0"
			frameborder="no" framespacing="0" allowfullscreen="true" sandbox="allow-same-origin allow-scripts"></iframe></div>`;
		};
    ...
    initMedia() {
        this.util.forEach(document.getElementsByClassName('media'), $media => {
            ...
            const $btn2 = $link.previousElementSibling;
            const $btn1 = $btn2.previousElementSibling;
...
$(document).on('ready pjax:end', function() {const theme = new Theme(); theme.init2();})
$(document).on('pjax:click', function() {window.config.data = {}; window.config.media={}; window.config.typeit.data = {};})
/*pjax ends*/